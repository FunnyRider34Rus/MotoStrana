...
import com.elpablo.motostrana.utils.CONST.FOLDER_USER_PROFIL_PHOTO
...
class ProfileFragment :
    ...
    private val documentPick =
        registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
            val path = remoteStorage.child(FOLDER_USER_PROFIL_PHOTO).child(USER.id)
            uri?.let {
    ...
    private fun editData() {
        with(binding) {
            ...
            if (name.isNotEmpty() && surname.isNotEmpty() && newNickname.isNotEmpty() && USER.nickname != newNickname) {
